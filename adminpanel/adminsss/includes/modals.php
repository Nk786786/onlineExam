      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="col-md-12">
            ...
            <div class="form-group">
              <label>Email</label>
              <input type="email" name="email" id="email" class="form-control" placeholder="Input Email" autocomplete="off" required pattern="[a-zA-Z0-9._%+-]+@gmail\.com$" title="Please enter a valid Gmail address with no missing characters" onkeyup="validateEmail()">
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
          <button type="submit" id="addButton" class="btn btn-primary" disabled>Add Now</button>
        </div>
      </div>
    </form>
  </div>
...
<script>
  function validateEmail() {
    ...
    // Check if the input value matches the pattern for a complete Gmail address
    if (emailInput.validity.valid) {
      addButton.disabled = false; // Enable the "Add Now" button
    } else {
      addButton.disabled = true; // Disable the "Add Now" button
    }
  }
</script>
